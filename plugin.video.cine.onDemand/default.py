...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[plugin.video.series.ON Listas-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Series.ON,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Series.ON,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

				
#def SKindex():
    #addon_log("SKindex")
    #addDir('Favorites','Favorites',4,'https://goo.gl/2p1AFg' ,  FANART,'','','','')
    #getData(_Edit.CHBase,'')
    #xbmcplugin.endOfDirectory(int(sys.argv[1]))

def CHIndex():
    addon_log("CHIndex")
    addDir('[B][COLOR orange] Buscar Listas[/B][/COLOR] ','Search',40,'http://cineondemand.net/ondemand/imagens/buscar.png' ,  FANART,'','','','')
    getData(_Edit.CHBase,'')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))	
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Series.ON,New source added.,5000,"+icon+")")
        if not url is None:
...
#hakamac code
    ...
    List=[]
    List.append(_Edit.CHBase)
    PassedUrls = 0
    FoundChannel = 1 
    ReadChannel = 0
    FoundMatch = 0
    progress = xbmcgui.DialogProgress()
    progress.create('Cine.onDemand Searching Please wait',' ')

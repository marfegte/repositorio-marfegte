...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(LiveStreamsPro,Nova lista adicionada.,5000,"+icon+")")
        if not url is None:
                ...
                if len(item('link')) >0:
                    ...
                    for i in item('link'):
                        if not i.string == None:
                            ## Añadido ######################################################
                            # Comprueba si tiene conector
                            if item('server'):
                                # Añade el nombre del conector al parámetro url con un separador
                                i.string+= "?server="+item('server')[0].string
                            #################################################################
                            url.append(i.string)
...
if mode==None:
    ...
    if not any(x in url for x in g_ignoreSetResolved):#not url.startswith("plugin://plugin.video.f4mTester") :
        ## Añadido ######################################################
        # Comprueba si necesita conector
        if "?server=" in url:
            # Separa la url y el server
            server = url.split("?server=")[1]
            url = url.split("?server=")[0]
